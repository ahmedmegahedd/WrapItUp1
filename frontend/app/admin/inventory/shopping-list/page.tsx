...
export default function ShoppingListPage() {
  ...
  const loadSuggested = useCallback(async () => {
    try {
      const res = await api.get('/admin/inventory/shopping-list')
      ...
      <div className="flex justify-between items-center mb-6 no-print">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            What to buy before your next prep day
          </p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <table className="w-full print-table">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-2 text-left text-xs font-medium text-gray-500 uppercase no-print w-10">
                    ✅
                  </th>
                  ...
                  <th className="px-6 py-2 text-left text-xs font-medium text-gray-500 uppercase no-print">
                    Actions
                  </th>
                </tr>
              </thead>
                      ...
                      <tr key={item.materialId} className="hover:bg-gray-50/50">
                        <td className="px-6 py-3 no-print">
                          <input
                            ...
                            disabled
                            readOnly
                            checked={showRestockInput}
                          />
                        </td>
                        <td className="px-6 py-3 font-medium">{item.name}</td>
                        <td className={`px-6 py-3 ${stockClass}`}>
                          {item.currentStock < 0
                            ? `${item.currentStock} ${item.unit}`
                            : `${item.currentStock} ${item.unit}`}
                        </td>
                        <td className="px-6 py-3">{item.suggestedQuantity}</td>
            ...
            <tbody className="divide-y divide-gray-200">
              {extras.length === 0 ? (
                ...
                extras.map((e) => (
                  <tr
                    key={e.id}
                    className={e.completed ? 'bg-gray-50 opacity-75' : ''}
                  >
                    <td className="px-6 py-3 no-print">
                      <input
                        type="checkbox"
                        checked={e.completed}
                        onChange={() => toggleExtraCompleted(e.id)}
                        className="rounded border-gray-300"
                      />
                    </td>
                    <td className="px-6 py-3">
                      {typeof window === 'undefined' ? (
                        ...
                        <input
                          ...
                          className="w-full px-2 py-1 border rounded text-sm no-print"
                        />
                      )}
                      {typeof window !== 'undefined' && (
                        <span className="print-only">{e.name || '—'}</span>
                      )}
                    </td>
                    <td className="px-6 py-3">
                      <input
                        ...
                        value={e.quantity}
                        onChange={(ev) =>
                          updateExtra(e.id, 'quantity', ev.target.value)
                        }
                        placeholder="0"
                        className="w-20 px-2 py-1 border rounded text-sm no-print"
                      />
                    ...
                    <td className="px-6 py-3">
                      <input
                        ...
                        value={e.unit}
                        onChange={(ev) =>
                          updateExtra(e.id, 'unit', ev.target.value)
                        }
                        placeholder="unit"
                        className="w-20 px-2 py-1 border rounded text-sm no-print"
                      />
                    ...
                    <td className="px-6 py-3">
                      <input
                        ...
                        value={e.note}
                        onChange={(ev) =>
                          updateExtra(e.id, 'note', ev.target.value)
                        }
                        placeholder="Note"
                        className="w-full px-2 py-1 border rounded text-sm no-print"
                      />
